...
void PrintBanner()
{
	...
	std::cout << std::endl;
	std::cout << "Version 0.2 - LumiaWOA (c) 2018-2020" << std::endl;
	std::cout << std::endl;
}
...
GUID WWAN_PROFILE_SET_ALL_2 = { 0x31A32D76, 0xE07F, 0x499D, {0x8F, 0xA3, 0xDD, 0xA7, 0x3B, 0xA7, 0x70, 0x57 } };
	...
	while (true)
	{
		...
		else
		{
			...
			for (int i = 0; i < (int)pIfList->dwNumberOfItems; i++)
			{
				...
				if (pIfInfo.InterfaceStatus.InterfaceState != WWAN_INTERFACE_STATE::WwanInterfaceStateNotReady)
				{
					NoSim = false;
				}
			}
